# Illustration of synchronization by GABAA synapses.
...
def alpha_n(v):
    return -0.01 * (v / brian2.mV + 34) / (np.exp(-0.1 * (v / brian2.mV + 34)) - 1)
...
def alpha_m(v):
    return -0.1 * (v / brian2.mV + 35) / (np.exp(-0.1 * (v / brian2.mV + 35)) - 1)
...
def rk4(state):
    ...
    new_state = add_k(state, add_k(add_k(add_k(k1, k2, 0.5), k3, 0.5), k4, 1),6)
    return new_state
...
def plot_raster(results):
    for i, v_trace in enumerate(results['v']):
        ...
        plt.plot(results['t_plot'][spike_times]*1000, [i] * len(spike_times), 'k.')
    plt.xlabel('Time (ms)')
    plt.ylabel('Neurons')
    plt.show()

def plot_potential(results):
    plt.figure()
    for i in range(100):
        plt.plot(results['t_plot']*1000,results['v'][i,:])
    plt.xlabel('Time (ms)')
...
def plot_s(results):
    plt.figure()
    ssavg = 1/P['n']*np.sum(results['s'],axis=0)
    plt.plot(results['t_plot']*1000,ssavg)
    plt.xlabel('Time (ms)')
    plt.ylabel('Summated synaptic drive')
    plt.show()

if __name__ == '__main__':
    results = run()
    # plot_raster(results)
    # plot_potential(results)
    plot_s(results)